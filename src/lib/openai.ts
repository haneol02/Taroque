...
"${question}"에 대한 답을 2-3문장으로 직접 전달합니다.
카드 이름을 나열하지 말고, 질문에 대한 타로의 시각을 명확하게 말하세요.
"카드들은 ~를 보여줍니다" 혹은 "지금 상황은 ~입니다" 형태로.

## 카드가 말하는 것

${cards.map(c => `### ${c.position}: ${c.cardName}${c.isReversed ? ' (역방향)' : ''}

먼저 이 카드가 타로에서 어떤 의미와 에너지를 가진 카드인지 1-2문장으로 설명합니다.
그 다음, "${question}"이라는 질문의 맥락에서, 이 자리(${c.position})에 이 카드가 나온 것이 질문자에게 구체적으로 무엇을 말하는지 2문장으로 이어서 설명합니다.${c.isReversed ? '\n역방향이므로, 이 카드의 에너지가 어떻게 억압되거나 내면화되어 있는지도 포함하세요.' : ''}`
  ).join('\n\n')}

## 흐름과 방향

카드들이 함께 그리는 이야기를 "${question}"의 관점에서 3-4문장으로.
"A카드와 B카드가..." 식으로 카드를 나열하지 말고, 상황의 흐름과 가능성을 하나의 문장으로 엮어내세요.
질문자가 놓치고 있거나 직면해야 할 것을 포함하세요.

## 지금 당신에게 필요한 것

- **해야 할 것**: 이 질문과 관련해 지금 당장 취해야 할 구체적인 행동이나 태도
- **내려놓을 것**: 상황을 막고 있는 집착, 두려움, 혹은 잘못된 믿음
- **주의할 것**: 이 시점에서 빠지기 쉬운 함정

## 리더의 말

질문자를 향한 마지막 한 마디. 카드 이야기 말고, 이 질문을 들고 온 사람에게 직접 말하듯이 2문장.

━━━ 작성 원칙 ━━━
- 카드 이름·상징 나열 금지. 질문자의 상황 이야기를 하세요
- "~카드는 ~를 의미합니다" 형태 사용 금지
- 역방향은 에너지의 내면화·지연·억압으로 해석
- 총 1300-1700자 내외, 한국어로 작성
`;
};
