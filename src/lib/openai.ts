...
이 카드가 "${question}"의 맥락에서 이 자리(${c.position})에서 무엇을 말하는지 2-3문장.
카드 이미지·상징은 한 문장 이내로 간략히 언급하고, 나머지는 질문자의 상황과 연결하세요.${c.isReversed ? '\n역방향이므로 이 에너지가 어떻게 막혀 있거나 내면화되어 있는지 포함하세요.' : ''}`
  ).join('\n\n')}
