    ...
    <div className={`relative group ${className}`}>
      <button
        ...
        className={`aspect-[3/5] rounded-lg transition-all duration-300 overflow-hidden w-full ${
          isSelected
            ? 'ring-2 ring-slate-400 scale-105 shadow-lg'
            : isDisabled
      ...
      >
        ...
        >
            ...
            {card && (
              <div className="w-full h-full relative">
                ...
                {isSelected && selectionNumber && (
                  <div className="absolute top-2 right-2 bg-slate-700/35 backdrop-blur-sm text-white text-xs px-2 py-1 rounded-full shadow-lg border border-slate-600/35 min-w-[24px] text-center opacity-0 group-hover:opacity-100 transition-opacity duration-200">
                    {selectionNumber}
                  </div>
                )}
              </div>
          ...
          <div className="absolute inset-0 w-full h-full backface-hidden">
            <div className="w-full h-full bg-gradient-to-br from-slate-800 to-slate-900 rounded-lg border border-slate-600 flex items-center justify-center relative overflow-hidden">
              {/* 배경 패턴 */}
              <div className="absolute inset-0 opacity-10">
                <div className="w-full h-full bg-repeat" style={{
                  backgroundImage: `url("data:image/svg+xml,%3Csvg width='40' height='40' viewBox='0 0 40 40' xmlns='http://www.w3.org/2000/svg'%3E%3Cg fill='%236b7280' fill-opacity='0.3'%3E%3Cpath d='M20 20c0 11.046-8.954 20-20 20v-40c11.046 0 20 8.954 20 20z'/%3E%3C/g%3E%3C/svg%3E")`
                }} />
              </div>
              ...
              {/* 테두리 장식 */}
              <div className="absolute inset-2 border border-slate-600/30 rounded-md" />
              <div className="absolute inset-4 border border-slate-600/20 rounded-sm" />
            </div>
          </div>
        </div>
      </button>
      ...
      {/* 호버 툴팁 */}
      {showTooltip && card && (
        <div className="absolute bottom-full left-1/2 transform -translate-x-1/2 mb-2 px-3 py-2 bg-slate-800 border border-slate-700 text-white text-xs rounded-lg opacity-0 group-hover:opacity-100 transition-opacity duration-200 pointer-events-none whitespace-nowrap z-10 shadow-lg">
